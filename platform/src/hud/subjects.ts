...
export interface Subject {
  ...
  toPromote?: string;
  /**
   * What this subject is called when it turns up in running prose.
   *
   * The Situation Report says "Shue et al. 1998" and "Tsyganenko 1989", not
   * "Shue et al. 1998 magnetopause" or "Tsyganenko T89c" — labels are written
   * for a list, aliases for a sentence. The report links the first occurrence
   * of one of these in a paragraph that has declared the subject, so a name
   * can never be linked by accident somewhere it was not meant.
   */
  aliases?: string[];
  /**
...
const MODELS: Subject[] = [
  ...
  {
    id: 'model.t89',
    aliases: ['Tsyganenko 1989 (T89c)', 'Tsyganenko 1989', 'T89'],
  ...
  },
  {
    ...
    aliases: ['Farris & Russell 1994', 'Farris \u0026 Russell'],
...
const LAYERS: Subject[] = [
  ...
  },
  {
    ...
    aliases: ['LASCO coronagraphs', 'LASCO'],
...
/**
 * The first sentence of a subject's limits.
 *
 * A section footer wants the headline caveat, not four sentences of it — the
 * full text is one click away in the card. Abbreviations with a full stop
 * inside them ("et al.", "e.g.") would split a sentence in the wrong place, so
 * the break has to be a full stop followed by a space and a capital.
 */
export function firstLimit(s: Subject): string {
  const m = /^(.+?[.!?])(\s+[A-Z(])/.exec(s.limits);
  return (m ? m[1]! : s.limits).trim();
}
